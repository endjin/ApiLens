...
try {
    $allStats = & "$apilens" stats --index "$indexPath" --format json | ConvertFrom-Json
    $allStats | Select-Object documentCount, totalSizeInBytes
} catch {
    Write-Host "Error getting all versions stats: $_" -ForegroundColor Red
}

Write-Host "`nLatest versions only:" -ForegroundColor Yellow
try {
    $latestStats = & "$apilens" stats --index ./nuget-latest-index --format json | ConvertFrom-Json
    $latestStats | Select-Object documentCount, totalSizeInBytes
} catch {
    Write-Host "Error getting latest versions stats: $_" -ForegroundColor Red
}
